...
## 实验步骤
### 步骤一 用户创建和空间分配
创建新的表空间和用户zpl，并分配给zpl表空间的查询权限以及创建视图的权限
#### 1） 创建表空间
...
```sql
CREATE USER zpl IDENTIFIED BY 123
...
* 给用户zpl分配表空间的使用配额
```sql
ALTER USER zpl QUOTA UNLIMITED ON USERS;
ALTER USER zpl QUOTA UNLIMITED ON USERS02;
```
* 分配角色CONNEC、RESOURCE，以便用户可以连接数据库，创建资源。
```sql
ALTER USER zpl ACCOUNT UNLOCK;
GRANT "CONNECT" TO zpl WITH ADMIN OPTION;
GRANT "RESOURCE" TO zpl WITH ADMIN OPTION;
```
*分配系统权限"CREATE VIEW"，以便创建视图

```sql
ALTER USER zpl DEFAULT ROLE "CONNECT","RESOURCE";
GRANT CREATE VIEW TO zpl WITH ADMIN OPTION;
```
![](p1.jpg)
### 步骤二 删除已经存在的表和序列
删除表和序列
...
![](p10.png)
##### ② 创建本地分区索引
在zpl的ORDERS表中创建本地分区索引ORDERS_INDEX_DATE
```sql
...
在zpl的ORDERS表中创建本地分区索引ORDERS_INDEX_CUSTOMER_NAME
```sql
...
![](p11.png)
##### ③ 设置唯一索引ODERS_PK
在zpl用户的ORDERS表中创建全局分区索引ORDERS_PK
```sql
